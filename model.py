...
def matrixify(vector, n):
    ...
    return T.repeat(T.shape_padleft(vector),
                    n if (theano.configdefaults.local_bitwidth() == 64) else T.cast(n, 'int32'), axis=0)
...
class Model(object):
    ...
    def setup_train(self):
        ...
        # Now time_result is a list of matrix [layer](time, batch/note, hidden_states) for each layer but we only care
        # about the hidden state of the last layer.
        ...
        # Now note_result is a list of matrix [layer](note, batch/time, onOrArticProb) for each layer, but we only care
        # about the hidden state of the last layer.
